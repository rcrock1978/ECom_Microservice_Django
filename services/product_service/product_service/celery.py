...
from catalog.infrastructure.processed_events import ProcessedEventStore
...
def register_consumers(bus: InMemoryMessageBus | None = None) -> InMemoryMessageBus:
	...
	def handle_inventory_adjusted(event: object) -> None:
		event_id = getattr(event, "event_id")
		if processed_store.is_processed(event_id):
			return
		processed_store.mark_processed(event_id)

	bus.register("inventory.adjusted", handle_inventory_adjusted)
	return bus
